...
tourney = []
event_id = [123491, 110486, 112209, 109725, 115896, 118026, 97059, 112546, 109862, 109752, 114367, 114083,
119757, 126120, 110369, 124519, 110928, 110504, 119872, 121400, 118370, 115845, 109577, 110970,
111543, 116279, 111101, 121542, 123616, 110776, 126350, 112660, 115669, 114288, 111064, 112518,
110205, 121747, 110220, 112250, 118814, 119490, 112428, 107675, 117267, 134026, 114036, 111779,
109431, 114904, 106833, 112969, 121203, 123236, 118568, 123631, 110560, 117141, 129705, 106335,
115061, 112259, 121546, 113077, 130390, 110383, 110384]


tourn_id = [14584, 13057, 13417, 12918, 13822, 14015, 11596, 13465, 12944, 12924, 13670, 13628, 13222, 14838,
13037, 14695, 13112, 12861, 14204, 14328, 14049, 13816, 12901, 13121, 13291, 13850, 13153, 14190,
14471, 13090, 14865, 13474, 13799, 13652, 13151, 13107, 13005, 14406, 13017, 13422, 14095, 14180,
13453, 12602, 13942, 15530, 13615, 13346, 12859, 13686, 12515, 13523, 14039, 14556, 14075, 14551,
13065, 13932, 15210, 12443, 13749, 13423, 13053, 13536, 15264, 13038, 13038]



bid_level = [2, 8, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 4, 4, 4, 4,
            4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 0, 0]
        ...
        if (('Bethel Park' in tourn or 'Mid America' in tourn or 'Golden Desert' in tourn) and count%2==1) or not('Bethel Park' in tourn or 'Mid America' in tourn or 'Golden Desert' in tourn):
            ...
            if(' & ' in names.text):
                ...
                for fullName in fullNames:
                    ...
                    firstFullName = firstFullName[0:firstFullName.index(' ')].strip() + ' ' + firstFullName[firstFullName.index(' '):].strip()
                    secondFullName = temp[index + 1:].strip().upper()
                    secondFullName = secondFullName[0:secondFullName.index(' ')].strip() + ' ' + secondFullName[secondFullName.index(' '):].strip()
                    print(firstFullName)
...
with open('2019-2020Individual.json', 'w') as outfile:
